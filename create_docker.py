...
if __name__ == '__main__' :
    ...
    for index in range(sw_num):
        ...
        os.system("sudo docker create -it --name=s{} --net=none --privileged -v /etc/localtime:/etc/localtime:ro satellite /bin/bash > /dev/null".format(index))
    ...
    with open(file=filePath + "/config/db_conf/db_deploy") as file:
        ...
        for db in db_data:
            os.system("sudo docker create -it --name=db{} --net=none --privileged -v /etc/localtime:/etc/localtime:ro database /bin/bash > /dev/null".format(db))
