...
if __name__ == '__main__' :
    ...
    for index in range(sw_num):
        ...
        os.system("sudo docker create -it --name=s{} --net=none --privileged -v /etc/localtime:/etc/localtime:ro -v {}/config:/home/config satellite /bin/bash > /dev/null".format(index, filePath))
    ...
    with open(file=filePath + "/config/db_conf/db_deploy") as file:
        ...
        for db in db_data:
            os.system("sudo docker create -it --name=db{} --net=none --privileged -v /etc/localtime:/etc/localtime:ro -v {}/config:/home/config database /bin/bash > /dev/null".format(db, filePath))
